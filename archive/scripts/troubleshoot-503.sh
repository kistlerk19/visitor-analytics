...
aws ecs describe-services --cluster visitor-analytics --services visitor-analytics --query 'services[0].{runningCount:runningCount,desiredCount:desiredCount,status:status,taskDefinition:taskDefinition}'
...
if [ -n "$TASK_ARNS" ]; then
    aws ecs describe-tasks --cluster visitor-analytics --tasks $TASK_ARNS --query 'tasks[0].{lastStatus:lastStatus,healthStatus:healthStatus,containers:containers[0].{name:name,lastStatus:lastStatus,healthStatus:healthStatus}}'
else
    echo "❌ No tasks found"
fi
...
TARGET_GROUP_ARN=$(aws elbv2 describe-target-groups --names visitor-analytics --query 'TargetGroups[0].TargetGroupArn' --output text 2>/dev/null || echo "")
...
aws logs tail /ecs/visitor-analytics --since 10m || echo "❌ No logs available"
